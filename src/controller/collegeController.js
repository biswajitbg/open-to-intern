...
const createCollege = async function (req, res) {
    try {
        ...
        if (!name) {
            return res.status(400).send({ msg: "Please enter alphabets only for abbrv and maximum legth should be 10" })
        }
        //<!-----------------Full name Regex------------------------->        
        ...
        if (!fullName) {
            return res.status(400).send({ msg: "Please enter alphabets only for full name of college and maximum length should be 50" })
        }
        //<!-----------------Logolink Regex------------------------->        
        if (!keyValid(data.logoLink)) return res.status(400).send({ status: false, message: "Please enter valid logo link" })
        logoLink = /^(http(s?):)([/|.|\w|\s|-])*\.(?:jpeg|png|jpg)$/.test(data.logoLink)
        if (!logoLink) {
            return res.status(400).send({ msg: "Please enter valid logo link" })
        }
...
const createIntern = async function (req, res) {
    try {
        ...
        if (!email) {
            return res.status(400).send({ msg: "Please enter valid format of email" })
        }
        ...
        if (!mobile) {
            return res.status(400).send({ msg: "Please enter valid format of mobile" })
        }
